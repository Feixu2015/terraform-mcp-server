...
func ensureClientInitialized(t *testing.T, client mcpClient.MCPClient) {
	...
	// Accept either legacy name or new name for backward compatibility
	require.True(t, result.ServerInfo.Name == "terraform-mcp-server" || result.ServerInfo.Name == "opentofu-mcp-server")
}
...
func cleanupAllTestContainers(t *testing.T) {
	...
	cmd := exec.Command("docker", "ps", "-q", "--filter", "ancestor=opentofu-mcp-server:test-e2e")
	output, err := cmd.Output()
	if err != nil {
